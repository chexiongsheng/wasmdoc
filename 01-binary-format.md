...
> **wasm3 实现参考**：[m3_parse.c - m3_ParseModule()](wasm3/source/m3_parse.c#L598) — 模块解析入口，验证魔数和版本号，按顺序解析各 Section。
...
> **wasm3 实现参考**：[m3_parse.c - ParseModuleSection()](wasm3/source/m3_parse.c#L519) — Section 分发表，根据 section id 调用对应的解析函数；[m3_parse.c#L631](wasm3/source/m3_parse.c#L631) — `sectionsOrder` 数组实现 Section 排序检查。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Type()](wasm3/source/m3_parse.c#L49) — 解析类型段，创建 `M3FuncType` 并通过 [m3_env.c - Environment_AddFuncType()](wasm3/source/m3_env.c#L89) 去重存入环境。类型结构定义见 [m3_function.h - M3FuncType](wasm3/source/m3_function.h#L17)。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Import()](wasm3/source/m3_parse.c#L138) — 解析导入段，按 `importKind` 分别处理函数/表/内存/全局变量导入。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Function()](wasm3/source/m3_parse.c#L118) — 解析函数声明段，通过 `Module_AddFunction` 将类型索引关联到函数。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Memory()](wasm3/source/m3_parse.c#L412) — 解析内存段；[m3_parse.c - ParseType_Memory()](wasm3/source/m3_parse.c#L24) — 解析 Limits 类型。内存数据结构见 [m3_env.h - M3Memory](wasm3/source/m3_env.h#L28)。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Global()](wasm3/source/m3_parse.c#L427) — 解析全局变量段，包括初始化表达式。全局变量结构见 [m3_env.h - M3Global](wasm3/source/m3_env.h#L50)。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Export()](wasm3/source/m3_parse.c#L195) — 解析导出段，将导出名关联到函数/全局变量/内存/表。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Start()](wasm3/source/m3_parse.c#L247) — 解析启动函数索引；[m3_env.c - m3_RunStart()](wasm3/source/m3_env.c#L395) — 执行启动函数。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Element()](wasm3/source/m3_parse.c#L280) — 解析元素段（仅记录位置）；[m3_env.c - InitElements()](wasm3/source/m3_env.c#L340) — 实例化时将函数引用填充到 `table0`。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Code()](wasm3/source/m3_parse.c#L296) — 解析代码段，为每个函数记录 `wasm`/`wasmEnd` 指针（零拷贝）。函数结构见 [m3_function.h - M3Function](wasm3/source/m3_function.h#L44)。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Data()](wasm3/source/m3_parse.c#L355) — 解析数据段；[m3_env.c - InitDataSegments()](wasm3/source/m3_env.c#L316) — 实例化时将数据复制到线性内存。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Custom()](wasm3/source/m3_parse.c#L502) — 解析自定义段，识别 `"name"` 段并调用 [ParseSection_Name()](wasm3/source/m3_parse.c#L459)；支持通过 [m3_env.c - m3_SetCustomSectionHandler()](wasm3/source/m3_env.c#L83) 注册自定义处理器。
