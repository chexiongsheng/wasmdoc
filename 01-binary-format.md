...
> **BNF 符号说明**：`+` 表示字段**依次拼接**（即一个 import 由 module名、字段名、描述符三部分顺序组成）；`|` 表示**多选一**（即 importdesc 根据 tag 字节选择其中一种变体）。Wasm 官方规范用 `|` 同时表示这两种含义，容易混淆，本文用 `+` 区分拼接语义。
...
> **wasm3 实现参考**：[m3_parse.c - ParseSection_Custom()](wasm3/source/m3_parse.c#L551) — 解析自定义段，识别 `"name"` 段并调用 [ParseSection_Name()](wasm3/source/m3_parse.c#L500)；支持通过 [m3_env.c - m3_SetCustomSectionHandler()](wasm3/source/m3_env.c#L80) 注册自定义处理器。
